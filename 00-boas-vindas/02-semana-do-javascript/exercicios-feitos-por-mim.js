...
const oddRandomNumbers = randomNumbers.filter(n => {
  if (n % 2 === 1) return n
})
...
const numbersBelow501 = crazyNumbers.filter(n => {
  if (n < 501) {
    console.log(n)
  }
})
...
const nameMoviesTarantino = tarantinoMovies.filter(movie => {
  if (movie.release < 2000) {
    console.log({ name: movie.name, release: movie.release })
  }
})
...
const nameTvShows = tvShows.filter(films => {
  console.log({ name: films.name })
})
...
const cartDescriptionNameGame = cart.filter(game => {
  return { name: game.name }
})
const cartGameName = cartDescriptionNameGame.map(game => {
  return `- ${game.name} \n`
})
