...
const invoke = (argOne, argTwo, argThree) => {
  const isSomeParameterUndefined = [argOne, argTwo, argThree].includes(
    undefined,
  )
...
const availiableSpacesMessage = (spaces, booksIn) => {
  const availiableSpaces = spaces - booksIn
  ...
  return `Só ${fitPluralOrSingular} mais ${availiableSpaces} ${bookPluralOrSingular}`
}
...
booksBox.addBooks = booksQuantity => {
  ...
  if (boxSpacesAreNotEnough) {
    return availiableSpacesMessage(spaces, booksBox.booksIn)
  }
