...
console.log(typeof convertToString(12))
...
console.log(getStringLength('Rickson Lucas'))
...
const converteToLowerCase = string => string.toLowerCase()
console.log(
  converteToLowerCase(
    'CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO',
  ),
...
console.log(getIndex('k', 'rickson lucas'))
...
const itemArray = 'Lucas'
const arrayNames = ['Rickson', 'Fernando', 'João']
console.log(isItemIncluded(itemArray, arrayNames))
...
const drinks = ['Beer', 'Coke', 'Fanta']
const foodsAndDrinks = concatArrays(foods, drinks)
...
console.log(removeLastItem(foodsAndDrinks))
...
const showNumbersInfo = (item, index, array) => {
  ...
  console.log(`O ${itemPosition}º do array [${itens}] é ${item}`)
}
...
function formatLikes(likers) {
  ...
  switch (totalLikes) {
    ...
    case 1:
      return `${likers[0]} curtiu isso`

    case 2:
      return `${likers[0]} e ${likers[1]} curtiram isso`

    case 3:
      return `${likers[0]}, ${likers[1]} e ${likers[2]} curtiram isso`

    default:
      const remainingLikes = totalLikes - 2
      return `${likers[0]}, ${likers[1]} e mais ${remainingLikes} pessoas curtiram isso`
  }
}

// Exemplos de uso
// console.log(formatLikes([])) // Ninguém curtiu isso
// console.log(formatLikes(['Rafael'])) // Rafael curtiu isso
// console.log(formatLikes(['Rafael', 'Ana'])) // Rafael e Ana curtiram isso
// console.log(formatLikes(['Rafael', 'Ana', 'Paulo'])) // Rafael, Ana e Paulo curtiram isso
// console.log(formatLikes(['Rafael', 'Ana', 'Paulo', 'Maria', 'João'])) // Rafael, Ana e mais 3 pessoas curtiram isso
...
const getLikesMessage = (names = []) => {
  ...
  switch (names.length) {
    ...
    case 3:
      return `${firstName}}, ${secondName} e ${thirdName} pessoas curtiram isso!`
    default:
      return `${firstName}}, ${secondName} e mais ${totalNamesMinusTwo} curtiram isso!`
  }
}

console.log(getLikesMessage(['Rickson', 'Kamilly', 'Gabrielly', 'Christian']))
